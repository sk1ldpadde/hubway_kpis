...
dag = DAG('DAGforhubway', default_args=args, description='KPI Pipeline',
...
def download_data(ds, **kwargs):
    ...
    # Define the path where data will be downloaded
    download_path = '/home/airflow/bike_share/csv'
    if not os.path.exists(os.path.dirname(download_path)):
        os.makedirs(os.path.dirname(download_path))

    # Download the dataset
    api.dataset_download_files(
        'acmeyer/hubway-data', path=download_path, unzip=True)


download_bike_share_data = PythonOperator(
...
merge_bike_share_csv_files = BashOperator(
    ...
    bash_command='cd /home/airflow/bike_share/csv&& rename "s/[ ^ a-zA-Z0-9_.-]/_/g" *.csv && csvstack *.csv > bike_share_{{ ds }}.csv',
    dag=dag,
)
...
kpi_gen = SparkSubmitOperator(
    ...
    conf={
        ...
        'spark.driver.extraClassPath': '/home/hadoop/hadoop/share/hadoop/common/lib/slf4j-log4j12-1.7.25.jar',
        'spark.executor.extraClassPath': '/home/hadoop/hadoop/share/hadoop/common/lib/slf4j-log4j12-1.7.25.jar',
    },
    application_args=['--year', '{{ macros.ds_format(ds, "%Y-%m-%d", "%Y")}}', '--month', '{{ macros.ds_format(ds, "%Y-%m-%d", "%m")}}',
                      '--day',  '{{ macros.ds_format(ds, "%Y-%m-%d", "%d")}}', '--hdfs_input_path', '/user/hadoop/hubway'],
    dag=dag
)

create_local_import_directory >> remove_old_csv >> clear_local_import_directory >> download_bike_share_data >> create_bike_share_csv_directory >> filter_and_copy_bike_share_csv >> merge_bike_share_csv_files >> create_hdfs_bike_share_partition_directory >> hdfs_upload_bike_share_data >> kpi_gen
create_HiveTable_bike_share_trips >> addPartition_HiveTable_bike_share_trips
